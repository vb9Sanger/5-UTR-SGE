...
| `valiant_meta` | Path to the **`meta.csv`** file — the [VaLiAnT](https://github.com/cancerit/VaLiAnT) oligo design output also used to run QUANTS (STEP THREE); one row per designed oligo, with columns including `oligo_name`, `gene_id`, `transcript_id`, `ref_chr`/`ref_strand`/`ref_start`/`ref_end`, `ref_seq`, `mut_position`, `ref`/`new` (ref/alt bases), `mut_type`, `mseq` (mutated sequence), and others |
