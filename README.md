...
```bash
Rscript run_maveqc_VB.R 
```
*where the input directory is the folder one level above the 'input' folder generated above. See the script itself for full usage instructions.*

#### Output:
QC output as generated by `run_maveqc_VB.R` (see script for details).
